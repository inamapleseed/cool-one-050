  ...
  add: function (product_id) {
    $.ajax({
      ...
      success: function (json) {
        ...
        if (
          $(".product_wishlist_" + product_id + " .fa").hasClass("fa-heart-o")
        ) {
          $(".product_wishlist_" + product_id + " .fa").removeClass(
            "fa-heart-o"
          );
    ...
    $.ajax({
      ...
      success: function (json) {
        ...
        if ($("body.short_hand").length) {
          ...
          if (
            getURLVar("route") == "enquiry/cart" ||
            getURLVar("route") == "enquiry/checkout" ||
            getURLVar("route") == "quickenquiry/checkout"
          ) {
...
function UpdateQuantity(t, n) {
  ...
  (i += 1 * (n ? 1 : -1)),
    1 > i && (i = 1),
    t.attr("value", i.toString()).val(i.toString());
}
  ...
  if ($(e).length) {
    ...
    $(e).on("keyup", function (r) {
      if (
        (r.preventDefault(),
        r.stopPropagation(),
        Number($(e).val()) && 6 == $(e).val().length)
      ) {
        var n = $(e);
        if (1 == n.length)
          "INPUT" == $(n[0]).prop("tagName") &&
            (n.parent().addClass("relative"),
            n
              .parent()
              .append(
                '<i class="loading-prefix-js fa fa-spinner fa-pulse absolute-center-right"></i>'
              ));
        t && t.abort(),
          (t = $.get(
            ...
            function (e) {
              "results" in e &&
                e.found > 0 &&
                ((block = e.results[0].BLK_NO),
                (street = e.results[0].ROAD_NAME),
                (building = e.results[0].BUILDING),
                "NIL" == street && (street = ""),
                "NIL" == building && (building = ""),
                a && l && s
                  ? ($(a).val(street), $(l).val(building), $(s).val(block))
                  : a && l
                  ? ($(a).val(block + " " + street), $(l).val(building))
                  : a &&
                    ((address = e.results[0].ADDRESS),
                    (address_only = address.replace(e.results[0].POSTAL, "")),
                    $(a).val(address_only))),
...
$(document).ready(function () {
  ...
  $('input[type="password"]').each(function () {
    $view_password =
      '<i style="[STYLE]" class="fa fa-eye pointer absolute view-password" aria-hidden="true" onclick="view_password(this);" ontouchstart="view_password(this);" ontouchend="view_password(this);" ></i>';
  ...
  $(".input-number").each(function () {
    $(this).keydown(function (e) {
      -1 !== $.inArray(e.keyCode, [46, 8, 9, 27, 13]) ||
        ...
        (88 == e.keyCode && (!0 === e.ctrlKey || !0 === e.metaKey)) ||
  ...
  (function (e) {
    e.fn.UItoTop = function (t) {
      var o = e.extend(
        ...
        n = "#" + o.containerID,
        i = "#" + o.containerHoverID;
      e("body").append(
        '<span id="' + o.containerID + '">' + o.text + "</span>"
      ),
        e(n)
          .hide()
          .click(function (t) {
            e("html, body").animate({ scrollTop: 0 }, o.scrollSpeed),
              t.preventDefault();
          })
          .prepend('<span id="' + o.containerHoverID + '"></span>')
          .hover(
            function () {
              e(i, this).stop().animate({ opacity: 1 }, 600, "linear");
            },
            function () {
              e(i, this).stop().animate({ opacity: 0 }, 700, "linear");
            }
          ),
        e(window).scroll(function () {
          ...
          void 0 === document.body.style.maxHeight &&
            ...
            t > o.min ? e(n).fadeIn(o.inDelay) : e(n).fadeOut(o.Outdelay);
        }),
        initializeMobileNav();
    };
  })(jQuery),
  $(document).delegate('[data-toggle="modal-content"]', "click", function (e) {
    e.preventDefault(), $("#modal-content-custom").remove();
    let t = $(this).text();
    $(this).attr("data-title").length && (t = $(this).attr("data-title")),
      $.ajax({
        url: $(this).attr("href"),
        type: "get",
        dataType: "html",
        success: function (e) {
          (html = '<div id="modal-content-custom" class="modal">'),
            ...
            (html +=
              '        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>'),
            (html += '        <h4 class="modal-title">' + t + "</h4>"),
            (html += "      </div>"),
            (html += '      <div class="modal-body">' + e + "</div>"),
            (html += "    </div"),
            (html += "  </div>"),
            (html += "</div>"),
            $("body").append(html),
