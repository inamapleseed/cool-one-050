<div class="product-gutter" id="product-<?=$product_id?>"> <?php /* product option in product component :: add product id to div  */ ?>
	...
	<a href="<?php echo $href;?>" class="overlay2"></a>
		<div class="product-image-block relative image-zoom-hover">
			...
			style="top:<?= $sticker ? '30px' : '0px' ?>; color: #fff; background-color: #ba1212;">
				<?= $text_sale; ?>
			</a>
			<?php } ?>
			<a 
      ...
      <?php if($quantity) { ?>
        <a href="<?php echo $href;?>" class="overlay"></a>
      <?php } else { ?>
        <div class="overlay-not-avail"></div>
      <?php } ?>
        ...
        <div class="cart-buttons <?= $quantity ? '':'hide'; ?>">
          ...
          <?php if(!$enquiry){ ?>
            ...
            <?php } ?>
          <?php }else{ ?>
			<button type="button" data-loading-text="<?= $text_loading; ?>" disabled class="btn btn-primary btn-cart btn-cart-<?= $product_id; ?>" data-product-id="<?= $product_id; ?>"><?= $button_cart; ?></button>
          <?php } ?>
	      </div>
        <h5 class="<?= $quantity ? 'hide':'text-not-avail'; ?>">Not Available</h5>
			</div>
		</div>
		...
		<div class="product-details product-price-<?=$product_id?>">
			<?php if ($price) { ?>
				<div class="price">
					...
					<?php } else { ?>
						<span class="price-new price-new-discount"><?= $special; ?></span>
