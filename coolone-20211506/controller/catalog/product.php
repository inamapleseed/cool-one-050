...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		// if (count(preg_grep('/^filter[\d]*/', array_keys($this->request->get))) > 0 || !$this->cache->get('product_np')) {
		if($this->config->get('config_product_next_previous')) {
			$this->cacheProducts();
		}
		// }
		...
		$data = array(
			...
			'text_sticker_success'          =>	$this->language->get('text_sticker_success'        ),
			'text_sticker'                	=>	$this->language->get('text_sticker'                ),
			'text_sticker_percentage'     	=>	$this->language->get('text_sticker_percentage'     ),
			'text_sticker_percentage_off' 	=>	$this->language->get('text_sticker_percentage_off' ),
			'text_sticker_percentage_on'  	=>	$this->language->get('text_sticker_percentage_on'  ),
			'text_sticker_percentage_help'	=>	$this->language->get('text_sticker_percentage_help'),
			'text_sticker_text'          	=>	$this->language->get('text_sticker_text'          ),
			'text_sticker_sticker'    		=>	$this->language->get('text_sticker_sticker'    	   ),
			'text_sticker_label'          	=>	$this->language->get('text_sticker_label'          ),
			'text_sticker_image'          	=>	$this->language->get('text_sticker_image'          ),
			'text_sticker_image_help'     	=>	$this->language->get('text_sticker_image_help'     ),
			'text_sticker_duration'       	=>	$this->language->get('text_sticker_duration'       ),
			'text_sticker_duration_help'	=>	$this->language->get('text_sticker_duration_help'  ),
		);
		...
		$stickers = $this->config->get('sticker'); //debug($stickers);

		if($stickers){
			foreach($stickers as $sticker){
				$thumb = $data['placeholder'];
				if(is_file(DIR_IMAGE . $sticker['image'])){
					$thumb = $this->model_tool_image->resize($sticker['image'], 100, 100);
				}
		...
		if (isset($this->request->get['filter_out_of_stock'])) {
			$filter_out_of_stock = $this->request->get['filter_out_of_stock'];
		} else {
			$filter_out_of_stock = null;
		}
		...
		if (isset($this->request->get['filter_out_of_stock'])) {
			$url .= '&filter_out_of_stock=' . $this->request->get['filter_out_of_stock'];
		}
		...
		array_multisort(array_column($data['categories'], 'name'), SORT_ASC, $data['categories']);
		...
		foreach ($results as $result) {
			...
			if(!empty($product_categories)) {
				$product_categories_text = implode('<br>', $product_categories);
			}
			...
			$data['products'][] = array(
				...
				'image'      => $image,
				'name'       => $result['name'],
				'categories' => $product_categories_text,
		...
		$data['text_in_stock'] = sprintf($this->language->get('text_in_stock'),$this->config->get('config_low_stock_quantity'));
		$data['text_out_stock'] = $this->language->get('text_out_stock');
		$data['text_low_stock'] = sprintf($this->language->get('text_low_stock'),$this->config->get('config_low_stock_quantity'));
		...
		$data['filter_out_of_stock'] = $filter_out_of_stock;
	...
	protected function getForm() {
		...
		$data['button_next'] = $this->language->get('button_next') . ' ' .  $this->language->get('text_product');
		$data['text_next'] = $this->language->get('button_next');
		$data['button_prev'] = $this->language->get('button_prev') . ' ' . $this->language->get('text_product');
		$data['text_prev'] = $this->language->get('button_prev');
		...
		$data['tab_product_1'] = $this->language->get('tab_product_1');
		$data['tab_product_2'] = $this->language->get('tab_product_2');
		$data['tab_product_3'] = $this->language->get('tab_product_3');
		...
		if (isset($this->request->get['product_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			...
			/* mod for next/previous page */
				$next_previous_ids = $this->getNextPrevious($this->request->get['product_id']);

				if ($next_previous_ids) {
					if($next_previous_ids['next']) {
						$data['next_product'] = $this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $next_previous_ids['next'] . $url, true);
					}
					if($next_previous_ids['prev']) {
						$data['prev_product'] = $this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $next_previous_ids['prev'] . $url, true);
					}
				}
			/* mod for next/previous page */
		}
		...
		foreach ($product_options as $product_option) {
			...
			if (isset($product_option['product_option_value'])) {
				foreach ($product_option['product_option_value'] as $product_option_value) {
					...
					// << OPTIONS IMAGE
                    $option_name = '';
                    $option_info = $this->model_catalog_option->getOptionValue($product_option_value['option_value_id']);
                    if($option_info){
		...
		foreach ($product_images as $product_image) {
            ...
            if(isset($product_image['product_image_id'])){
                $product_option_images = $this->model_catalog_product->getProductOptionImages($product_image['product_image_id']);
            } else {
			...
			$data['product_images'][] = array(
				...
				// >>OPTIONS IMAGE
                'option_image' => $product_option_images
            	// >>OPTIONS IMAGE
			);
		}
		...
		if (isset($this->request->post['product_pdf_id'])) {
					$data['product_pdf_id'] = $this->request->post['product_pdf_id'];
		} elseif (!empty($product_info)) {
			$data['product_pdf_id'] = $product_info['product_pdf_id'];
		} else {
			$data['product_pdf_id'] = '';
		}
	...
	public function changeStatus() {
		...
		$this->model_catalog_product->changeStatus($product_id);
	}
	...
	private function cacheProducts() {
		...
		$products = $this->model_catalog_product->getNextPrevIds($filter_data);

		$product_np_a = array();
		foreach ($products as $product) {
			$product_np_a[] = $product['product_id'];

		}

		$this->cache->set('product_np',$product_np_a);
	}
	...
	private function getNextPrevious($product_id) {
		...
		if ($product_np_cache) {
			...
			$key = array_search($product_id,$product_np_cache);
	...
	public function manualAddProducts() {
		for ($c = 1; $c < 4 ; $c ++ ) {
			$sql = "INSERT INTO ".DB_PREFIX."product SET model = '-', sku = '', upc = '', ean = '', jan = '', isbn = '', mpn = '', location = '', quantity = '999', minimum = '1', subtract = '1', stock_status_id = '6', date_available = '2019-12-02', manufacturer_id = '0', shipping = '1', price = '20', points = '0', weight = '0', weight_class_id = '1', length = '0', width = '0', height = '0', length_class_id = '1', status = '1', tax_class_id = '0', sort_order = '1', date_added = NOW(), date_modified = NOW()";

			$query = $this->db->query($sql);
			$product_id = $this->db->getLastId();

			$this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '1', name = 'Product ".$c."', description = 'Lorem Ipsum', tag = '', meta_title = '', meta_description = '', meta_keyword = ''");

			$this->db->query("UPDATE " . DB_PREFIX . "product SET image = 'catalog/Banner/slider1_m.jpg' WHERE product_id = '" . (int)$product_id . "'");
